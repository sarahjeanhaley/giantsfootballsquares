...
class ReverseSub(Sub):
    ...
    def _resolve(self) -> Callable or None:
        ...
        """
        # Historically, we have passed refs relative to the Sub the ReverseSub is on.
        # This has some advantages and disadvantages. Let's keep it the way it has been.
        root = getattr(self._root, "_root", self._hub)
        root_path = []
        while not isinstance(root, Hub):
            root_path.insert(0, root._subname)
            root = root._root
        ignore_n_refs = len(root_path)
        ref = ".".join(self._refs[ignore_n_refs:])
    ...
    def __call__(self, *args, **kwargs):
        ...
        # reverse subs are used for API calls.
